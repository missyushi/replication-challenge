...
Version 1 	
	...
	WHAT NEEDS TO BE DONE (March 4, 2021)
		...
		- Add pre-COVID means 
		- Completely change the tables. 
	
Version 2
	Splitting the code into something which creates two tables 
	
Version 3
	Deleting the end scripts which are no longer useful 
	Cleaned up the title and the table formats to make it nearly done 
	
Version 4
	Tidying up the standard errors for the Canada regressions 
	
Version 5
	Adding bootstrapped standard errors to the tables 
	Adding pre-treatment means to the tables 
	
Version 6
	Trying to add more pre-Treatment Means to the tables. 
	
Version 7
	Updated .dta file (March 30, 2021)
...
/* Looping over panels (geography) */	
	...
	/* Create top panel */
		...
		if `panelNum' == 1{
			...
			esttab est1 est2 est3 est4 est5 est6 using "ReplicationFolder_CJE_Covid/Tables/March_beforeAndAfter_Canada.tex" ///
				, 	replace ///
				...
				prehead(`"\begin{tabular*}{\textwidth}{ @{\extracolsep{\fill}}l*{@span}{c}}"' `"\hline\hline"' `"\\"' `" &\multicolumn{6}{c}{Canada, Labour Force Survey} \\"' `"\cline{2-7} \\"' `"&\multicolumn{3}{c}{Unemployment} &\multicolumn{3}{c}{Labour Force Participation} \\"' `"\cline{2-4} \cline{5-7} \\"' ) ///
				posthead(`"\hline"' `"\\"') ///
				prefoot(`"\\"') ///
		...
		/* Export the table to LaTeX */	
			esttab matrix(addedInfo, fmt(%9.4f)) ///
				using "ReplicationFolder_CJE_Covid/Tables/March_beforeAndAfter_Canada.tex" ///
				, 	append ///
				label noisily ///
				compress /// 
	...
	/* Create middle panel */
		if `panelNum' == 2 {
			...
			// Middle Panel Panel
			esttab using "ReplicationFolder_CJE_Covid/Tables/March_beforeAndAfter_QcOnAbBc.tex" ///
				, 	replace ///
				label keep(1.postCovid*) noisily nostar ///
				b(%9.3f) se(%9.3f)  compress eqlabels(none) noomit numbers /// 
				varwidth(50) modelwidth(4) wrap nobase nodepvars ///
				title(, none) /// 
				prehead(`"\begin{tabular*}{\textwidth}{ @{\extracolsep{\fill}}l*{@span}{c}}"' `"\hline\hline"' `"\\"' `" &\multicolumn{6}{c}{Labour Force Survey} \\"' `"\cline{2-7} \\"' `"&\multicolumn{3}{c}{Unemployment} &\multicolumn{3}{c}{Labour Force Participation} \\"' `"\cline{2-4} \cline{5-7} \\"' ) ///
