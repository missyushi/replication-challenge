...
/* Show in LaTeX */
	esttab using "ReplicationFolder_CJE_Covid/Tables/March_SummaryStatistics_indexes_noc10.tex" ///
		, 	replace /// 
		noisily compress label /// 
		eqlabels(`e(labels)', begin(\hspace{0.25cm})) varwidth(50) modelwidth(4) wrap ///
		varlabels(_cons Constant, nolast) /// 
		mlabels(, none) nobase nodepvars nonumbers noobs nomtitle ///
...
Directions for perfectly recreating the tables in LaTeX 
	- delete row 8 
	- Put parentheses around the s.d.
	- Wrap the text and complete the names of the groups 
	- Comment out the Total rows
	- add \\[0.25em] to between the subgroups 
*/
