...
Version 1 
	- First Pass 
	- Added additional restrictions to omit all health care workers in the 
		regressions 
...
/* THE BELOW ARE RESTRICTIONS ON THE WAGES AND HOURS, RESPECTIVELY 
	Note that it also restricts the sample to non-Health Care Workers 
*/
	local  regConditions1 "if (K_age_12 <= 10) & (K_lfsstat != 3) & (K_cowmain <= 2) & K_noc_10 != 2" // only observations we have $ for 
	local  regConditions2 "if (K_age_12 <= 10) & (K_lfsstat != 3) & (K_cowmain <= 2) & K_noc_10 != 2" // only observations we have $ for
...
/* Create a pretty table */
	esttab using "ReplicationFolder_CJE_Covid/Tables/March_indexesInteracted_nonHealthCare_appendix.tex" ///
		, 	replace ///
		...
		delimiter("	&") ///
		prehead(`"\begin{tabular*}{\textwidth}{ @{\extracolsep{\fill}}l*{@span}{c}}"' `"\hline\hline"' `"\\"' `" &\multicolumn{4}{c}{Canada, Labour Force Survey} \\"' `" &\multicolumn{4}{c}{Non-Health Care Workers} \\"' `"\cline{2-5} \\"' `"&\multicolumn{2}{c}{Real} &\multicolumn{2}{c}{Total Actual} \\"' `"&\multicolumn{2}{c}{Hourly Wage} &\multicolumn{2}{c}{Hours Worked Weekly} \\"' `"\cline{2-3} \cline{4-5} \\"' ) ///
		posthead(`"\hline"' `"\\"') ///
		prefoot(`"\\"') ///
